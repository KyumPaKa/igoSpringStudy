...
@Service
@Transactional
public class UnknownBoardServiceImpl implements BoardService {

	@Resource(name="unknownBoardDAOImpl")
	BoardDAO boardDao;
		...
		try {
			...
			for(MultipartFile filePart : fileList) {
				...
				if (originalFileName != "") {
					...
					if(idx != -1) {
						if(idx == 0) attachDTO.setRefIdx(boardDao.countArticle2("all", "", 3) + 1);
						else attachDTO.setIdx(idx);
