...
$(function() {
	...
	$("#btnSearch").click(function() {
		$("#searchForm").attr("action", "${path}/fb/list.do");
		$("#searchForm").submit();
	});
});
function aTitle(idx) {
	location.href = "${path}/fb/view.do?refIdx=${refIdx}&idx=" + idx + "&search_option=${search_option}&keyword=${keyword}";
}
function aTitleBlack(idx) {
	location.href = "${path}/qa/view.do?refIdx=${refIdx}&idx=" + idx + "&search_option=${search_option}&keyword=${keyword}";
}
function aFirst() {
	location.href = "${path}/fb/list.do?refIdx=${refIdx}&curPage=1&search_option=${search_option}&keyword=${keyword}";
}
function aPrev() {
	location.href = "${path}/fb/list.do?refIdx=${refIdx}&curPage=${pager.prevPage}&search_option=${search_option}&keyword=${keyword}";
}
function aNow(cnt) {
	location.href = "${path}/fb/list.do?refIdx=${refIdx}&curPage=" + cnt + "&search_option=${search_option}&keyword=${keyword}";	
}
function aPost() {
	location.href = "${path}/fb/list.do?refIdx=${refIdx}&curPage=${pager.nextPage}&search_option=${search_option}&keyword=${keyword}";
}
function aLast() {
	location.href = "${path}/fb/list.do?refIdx=${refIdx}&curPage=${pager.totPage}&search_option=${search_option}&keyword=${keyword}";
}
...
<c:choose>
	...
	<tr>
		<td>${list.rn}</td>
		<td align="left">
			<c:if test="${list.secretCheck == 1}">
			[비밀글]
			</c:if>
			<sec:authorize access="hasRole('ROLE_BLACK')">
			<a onclick="aTitleBlack('${list.idx}')" href="#">${list.title}</a>
			</sec:authorize>
		...
		<td colspan="4">
			<form id="searchForm">
				<select name="search_option" style="width: 100px; height: 30px;">
					...
					<option value="refWriter" <c:out value="${search_option=='refWriter'?'selected':''}" />>작성자</option>
				</select>
